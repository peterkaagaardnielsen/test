...
#define FW_ENTRY_ARM  0x00020000 // This is where the Application is stored (it's reset vector)...
#define FW_VERSION_ARM (FW_ENTRY_ARM+0x40) // This is where the fw version info is stored (a 4 byte unsigned int)
#define FW_SIZE_ARM    (FW_ENTRY_ARM+0x44) // This is where the fw size info is stored (a 4 byte unsigned int)
#define FW_CRC_ARM     (FW_ENTRY_ARM+0x48) // This is where the fw crc info is stored (a 4 byte unsigned int)
#define FW_TARGET_ARM  (FW_ENTRY_ARM+0x4C) // This is where the fw target info is stored (a 4 byte unsigned int)


#define FW_MAX_SIZE   (368*1024)           // Maximum size of firmware
...
typedef struct {
  ...
  unsigned int target;  // Type of target the firmware is intended for (7=blackbird, 8= robin etc)
